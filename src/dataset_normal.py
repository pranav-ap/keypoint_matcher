...
class MatchesDataset(torch.utils.data.Dataset):
    ...
    def _get_names(self):
        names = []
        ds_types = ['all', 'normal', 'blur', 'all_missing']
        ...
        for ds_type in ds_types:
            ...
            for video in config.task.videos[self.stage][ds_type]:
                ...
                for cam in config.task.cams:
                    ...
                    if len(references.items()) == 0 or len(references.items()) == 1:
                        continue
                    ...
                    for a, b in zip(references.items(), targets.items()):
                        ...
                        if len(ref_dataset[()]) == 0 or len(ref_dataset[()]) == 1:
                            continue
