...
def match_collate_fn(batch):
    ref_patches, references, tar_patches, targets, estimates, certainties, tar_patches_fake, targets_fake, estimates_fake = zip(*batch)
    ...
    targets_fake = torch.tensor(targets_fake, dtype=torch.float32)
    estimates_fake = torch.tensor(estimates_fake, dtype=torch.float32)
    ...
    return (
        ...
        estimates,
        certainties,
        torch.cat(tar_patches_fake, dim=0).unsqueeze(1),
        targets_fake,
        estimates_fake, 
    )
...
class MatchesDataset(torch.utils.data.Dataset):
    ...
    def _prepare_image(self, idx):
        ...
        tar_patch_fake, target_fake, guess_fake = prepare_jitter_target_patch(
            tar_image, tar_keypoint_fake, guess_keypoint_fake,
            patch_size=config.image.patch_size,
            stage=self.stage,
            valid=False,
        )
        ...
        if self.image_augmentation_no_kp:
            ...
            patch_np = np.array(tar_patch_fake)
            transformed = self.image_augmentation_no_kp(image=patch_np)
            tar_patch_fake = Image.fromarray(transformed['image'])
        ...
        if self.patch_normalize:
            ...
            tar_patch_fake = self.patch_normalize(tar_patch_fake)
            # tar_patch_fake = min_max_normalize(tar_patch_fake, min_val=0.0, max_val=1.0)
        ...
        assert 0 <= target_fake[0] < config.image.patch_size, f"Target X-coordinate out of bounds: {target_fake[0]} (Expected between 0 and {config.image.patch_size})"
        assert 0 <= target_fake[1] < config.image.patch_size, f"Target Y-coordinate out of bounds: {target_fake[1]} (Expected between 0 and {config.image.patch_size})"
        assert 0 <= guess_fake[0] < config.image.patch_size, f"Guess X-coordinate out of bounds: {guess_fake[0]} (Expected between 0 and {config.image.patch_size})"
        assert 0 <= guess_fake[1] < config.image.patch_size, f"Guess Y-coordinate out of bounds: {guess_fake[1]} (Expected between 0 and {config.image.patch_size})"
        
        return ref_patch, reference, tar_patch, target, guess, certainty, tar_patch_fake, target_fake, guess_fake
