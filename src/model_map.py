...
class MatcherModel(nn.Module):
    def __init__(self):
        ...
        self.positional_encoding = positionalencoding2d(out_channels, height=config.image.train_patch_size, width=config.image.train_patch_size).unsqueeze(0).to(device)
        ...
        self.backbone = nn.Sequential(
            ...
            ResNetBlock(256, out_channels, stride=1),
        )

        self.positional_encoding2 = positionalencoding2d(out_channels, height=config.image.train_patch_size, width=config.image.train_patch_size).unsqueeze(0).to(device)
        ...
        self.confidence_head = nn.Sequential(
            ResNetBlock(out_channels, 512, stride=1),
            ResNetBlock(512, 1, stride=1),
            # nn.Sigmoid(),
        )
    ...
    def forward(self, reference_patches, target_patches, reference_coords):
        ...
        reference_coords = reference_coords / (config.image.train_patch_size - 1) # 31.0 127.0
        ...
        x = x + self.positional_encoding2.clone()
